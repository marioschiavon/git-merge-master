...
const READ_STORAGE_KEY = "inbox:lastViewedAt";
...
function isUnread(c: InboxConversation, lastViewed: Record<string, number>): boolean {
  if (!c.last_message || c.last_message.direction !== "inbound") return false;
  const sent = new Date(c.last_message.sent_at).getTime();
  return sent > (lastViewed[c.id] || 0);
}
...
export default function Inbox() {
  ...
  const seenMessagesRef = useRef<Map<string, string | undefined>>(new Map());
  const firstRunRef = useRef(true);

  const markViewed = useCallback((id: string) => {
    setLastViewed((prev) => {
      const next = { ...prev, [id]: Date.now() };
      try { localStorage.setItem(READ_STORAGE_KEY, JSON.stringify(next)); } catch {}
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    if (selectedId) {
      setSearchParams((p) => { p.set("conversation", selectedId); return p; }, { replace: true });
      markViewed(selectedId);
    }
  }, [selectedId, setSearchParams, markViewed]);

  // Toast quando chega mensagem nova em outra conversa
  useEffect(() => {
    if (firstRunRef.current) {
      for (const c of queue) seenMessagesRef.current.set(c.id, c.last_message?.sent_at);
      firstRunRef.current = false;
      return;
    }
    for (const c of queue) {
      const prev = seenMessagesRef.current.get(c.id);
      const cur = c.last_message?.sent_at;
      if (cur && cur !== prev && c.last_message?.direction === "inbound" && c.id !== selectedId) {
        toast.message(`Nova mensagem de ${c.lead?.name || "lead"}`, {
          description: c.last_message.content.slice(0, 80),
          action: { label: "Abrir", onClick: () => setSelectedId(c.id) },
        });
      }
      seenMessagesRef.current.set(c.id, cur);
    }
  }, [queue, selectedId]);

  const filtered = useMemo(() => {
    let list = queue;
    if (filter === "mine") list = list.filter((c) => c.human_taken_by === user?.id);
    if (filter === "unread") list = list.filter((c) => isUnread(c, lastViewed));
    return list;
  }, [queue, filter, user?.id, lastViewed]);

  const unreadCount = useMemo(() => queue.filter((c) => isUnread(c, lastViewed)).length, [queue, lastViewed]);
      ...
      <div className="px-6 py-4 border-b">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            {unreadCount > 0 && (
              <Badge className="text-xs bg-primary text-primary-foreground">{unreadCount} não lidas</Badge>
            )}
            {slaBreached > 0 && (
            ...
            <div className="flex rounded-md border overflow-hidden text-xs">
              ...
              >Meus</button>
              <button
                className={cn("px-3 py-1 border-l", filter === "unread" ? "bg-primary text-primary-foreground" : "bg-background hover:bg-muted")}
            ...
            {isLoading ? (
              ...
              <div className="divide-y">
                {filtered.map((c) => {
                  ...
                  const unread = isUnread(c, lastViewed);
                  const slaOver = c.last_inbound_at && Date.now() - new Date(c.last_inbound_at).getTime() > 15 * 60_000;
                  const activityTs = c.latest_activity_at || c.last_inbound_at;
                  return (
                    <button
                      key={c.id}
                      onClick={() => setSelectedId(c.id)}
                      className={cn(
                        "w-full text-left p-3 hover:bg-muted/50 transition border-l-2",
                        isActive ? "bg-muted border-l-primary" : slaOver ? "border-l-amber-400" : "border-l-transparent",
                      )}
                    >
                            ...
                            <div className="flex items-center gap-1.5 shrink-0">
                              {activityTs && (
                                <span className="text-[10px] text-muted-foreground whitespace-nowrap">
                                  {formatDistanceToNow(new Date(activityTs), { locale: ptBR, addSuffix: false })}
                                </span>
                              )}
                              {unread && <span className="h-2 w-2 rounded-full bg-primary" aria-label="não lida" />}
                            </div>
                          </div>
        ...
        <div className="flex flex-col overflow-hidden border-r">
          {selected ? (
            <ChatPanel
              ...
              text={text}
              setText={setText}
              onActivity={() => markViewed(selected.id)}
            />
          ) : (
...
function ChatPanel({
  ...
  // Auto-scroll só se já estava perto do fim
  useEffect(() => {
  ...
  // Marca como vista sempre que houver mensagens novas e o painel estiver aberto
  useEffect(() => {
    if (messages.length > 0) onActivity();
