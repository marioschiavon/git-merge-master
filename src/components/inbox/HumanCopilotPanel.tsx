...
import {
  Loader2, Sparkles, CalendarPlus, CalendarX, CalendarClock, Send, Bot,
  MessageSquareReply, CalendarCheck, RefreshCcw,
} from "lucide-react";
...
export function HumanCopilotPanel({
  ...
  const [bookGuests, setBookGuests] = useState<string[]>([]);
  ...
  const [reGuests, setReGuests] = useState<string[]>([]);
  const [reNotify, setReNotify] = useState(true);
  ...
  const handleReschedule = async () => {
    ...
    setBusy("reschedule");
    try {
      const { data, error } = await supabase.functions.invoke("human-reschedule-booking", {
        body: {
          conversation_id: conversationId,
          booking_uid: activeBooking.data?.calcom_booking_uid,
          start: reStart,
          reason: reReason || undefined,
          guests: reGuests,
          notify_lead: reNotify,
        },
      });
      if (error) throw new Error(error.message);
      if ((data as any)?.error) throw new Error((data as any).error);
  ...
  const activeMeetingLabel = activeBooking.data?.scheduled_at
    ? format(new Date(activeBooking.data.scheduled_at), "EEEE, d 'de' MMMM 'às' HH:mm", { locale: ptBR })
    : null;
    ...
    <Card className="border-primary/20">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {activeMeetingLabel && (
            <div className="rounded-md border bg-primary/5 px-2 py-1.5 text-[11px] flex items-center gap-1.5">
              <CalendarCheck className="h-3 w-3 text-primary" />
              <span className="truncate">Reunião ativa: <strong>{activeMeetingLabel}</strong></span>
            </div>
          )}

          <Tabs defaultValue="suggest" className="w-full">
            <TabsList className="grid grid-cols-3 h-8">
              <TabsTrigger value="suggest" className="text-[11px]">Sugerir</TabsTrigger>
              <TabsTrigger value="book" className="text-[11px]">Agendar</TabsTrigger>
              <TabsTrigger value="resched" className="text-[11px]" disabled={!activeBooking.data}>Remarcar</TabsTrigger>
            </TabsList>
            ...
            {/* Agendar */}
            <TabsContent value="book" className="space-y-2 mt-2">
              <DaySlotPicker
                conversationId={conversationId}
                selectedStart={bookStart}
                onSelectStart={setBookStart}
              />
              <div className="space-y-1">
                <Label className="text-[11px] text-muted-foreground">Convidados extras</Label>
                <GuestsInput value={bookGuests} onChange={setBookGuests} />
              </div>
              <div className="space-y-1">
                <Label className="text-[11px] text-muted-foreground">Anotação (opcional)</Label>
                <Textarea
                  value={bookNotes}
                  onChange={(e) => setBookNotes(e.target.value)}
                  placeholder="Contexto que aparece no convite"
                  className="text-xs min-h-[60px]"
                />
              </div>
              <div className="flex items-center justify-between">
                <Label htmlFor="notify-book" className="text-[11px]">Avisar lead pelo chat</Label>
                <Switch id="notify-book" checked={bookNotify} onCheckedChange={setBookNotify} />
              </div>
              <Button
                size="sm"
                className="w-full"
                disabled={!bookStart || busy === "book-direct"}
                onClick={handleBookDirect}
              >
                {busy === "book-direct" ? <Loader2 className="h-3.5 w-3.5 mr-2 animate-spin" /> : <CalendarCheck className="h-3.5 w-3.5 mr-2" />}
                Confirmar agendamento
              </Button>
            </TabsContent>

            {/* Remarcar */}
            <TabsContent value="resched" className="space-y-2 mt-2">
              {!activeBooking.data ? (
                <p className="text-[11px] text-muted-foreground">Sem reunião ativa para remarcar.</p>
              ) : (
                <>
                  <DaySlotPicker
                    conversationId={conversationId}
                    selectedStart={reStart}
                    onSelectStart={setReStart}
                  />
                  <div className="space-y-1">
                    <Label className="text-[11px] text-muted-foreground">Convidados extras</Label>
                    <GuestsInput value={reGuests} onChange={setReGuests} />
                  </div>
                  <div className="space-y-1">
                  ...
                  <div className="flex items-center justify-between">
                    <Label htmlFor="notify-re" className="text-[11px]">Avisar lead pelo chat</Label>
                    <Switch id="notify-re" checked={reNotify} onCheckedChange={setReNotify} />
                  </div>
                  <Button
                    size="sm"
                    className="w-full"
                    disabled={!reStart || busy === "reschedule"}
                    onClick={handleReschedule}
                  >
                    {busy === "reschedule" ? <Loader2 className="h-3.5 w-3.5 mr-2 animate-spin" /> : <RefreshCcw className="h-3.5 w-3.5 mr-2" />}
                    Remarcar reunião
                  </Button>
                </>
              )}
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant="outline" size="sm" className="w-full justify-start text-red-600 hover:text-red-700" disabled={!activeBooking.data}>
