...
import { tryGetCompanyCalcomCreds } from "../_shared/calcom.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    // Resolve API key per company once, then group holds by lead as before.
    const companyKeyCache = new Map<string, string | null>();
    const getCompanyKey = async (companyId: string): Promise<string | null> => {
      if (companyKeyCache.has(companyId)) return companyKeyCache.get(companyId)!;
      const creds = await tryGetCompanyCalcomCreds(supabase, companyId);
      const k = creds?.apiKey ?? null;
      companyKeyCache.set(companyId, k);
      return k;
    };
    ...
    for (const [leadId, holds] of Object.entries(holdsByLead)) {
      try {
        ...
        const apiKey = companyId ? await getCompanyKey(companyId) : null;

        for (const hold of holds) {
          if (hold.cal_booking_uid && apiKey) {
            try {
