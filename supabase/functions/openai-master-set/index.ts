...
serve(async (req) => {
  ...
  try {
    ...
    // Validate against api.openai.com /v1/models — a minimal auth probe.
    ...
    if (probe.status === 401 || probe.status === 403) {
      ...
      return jsonResponse(
        { ok: false, status: probe.status, message: `Chave rejeitada pelo OpenAI (${probe.status}): ${text.slice(0, 300)}` },
        200,
        corsHeaders,
      );
    }
    ...
    if (!passphrase) {
      return jsonResponse(
        { ok: false, message: "RESEND_KEY_PASSPHRASE não configurado" },
        500,
        corsHeaders,
      );
    }
    ...
    const { error } = await admin.rpc("set_openai_master_key", {
      _api_key: apiKey,
    ...
    return jsonResponse({ ok: true, message: "Chave OpenAI salva e validada." }, 200, corsHeaders);
  } catch (e) {
